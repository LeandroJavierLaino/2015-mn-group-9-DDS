...
class Usuario extends Entity {
	...
	//Loggeo
	static Logger log = Logger.getLogger(typeof(Usuario).getName)
	...
	def postProcesarRecetas() {
		...
		if(recetasFiltradas.size >=  100){
			log.info("Se realizó una consulta con 100 resultados o más hecha por el usuario:" + this.nombre)
		}		
		if (habilitaSusFavoritos()) {
			marcarComoFavoritas(recetasFiltradas)
		}
	...
	def marcarComoFavoritas(Set<Receta> recetas) {
		for (receta : recetas) {
	...
	def esUnUsuarioAAvisarPorMail(Usuario usuario){
		usuariosAEnviarMail.contains(usuario)
	}
