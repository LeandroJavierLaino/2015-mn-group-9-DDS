    ...
    if (_and) {
      ...
      final Consumer<Receta> _function = new Consumer<Receta>() {
        public void accept(final Receta receta) {
          boolean _containsKey = MonitorRecetasM.this.contadorDeRecetas.containsKey(receta);
          if (_containsKey) {
            Integer _get = MonitorRecetasM.this.contadorDeRecetas.get(receta);
            Integer _get_1 = MonitorRecetasM.this.contadorDeRecetas.get(receta);
            int _plus = ((_get_1).intValue() + 1);
            MonitorRecetasM.this.contadorDeRecetas.replace(receta, _get, Integer.valueOf(_plus));
          } else {
            MonitorRecetasM.this.contadorDeRecetas.put(receta, Integer.valueOf(1));
          }
        }
      };
